...
class ProfessionalController extends Controller
{
        ...
        try {
            ...
            return response()->json([
                'data' => [
                    ...
                    'extencion' => sizeof($oportunidades)
                ]
            ], 200);
    ...
    function unlinkOffer(Request $request)
    {
        try {
            $professional = Professional::findOrFail($request->professional_id);
            $status = Catalogue::where('code', '2')->where('type', 'offer_professional.status')->first();
            $professional->offers()->updateExistingPivot($request->id, ['status_id' => $status->id]);
